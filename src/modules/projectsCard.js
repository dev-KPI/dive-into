...
import imgModalUser from "/assets/img/modal-img.png";
  class ProjectsCard {
    constructor(id, title, description, members, githubLink, languages, specialization) {
      this.id = id;
      this.title = title;
      ...
      el.innerHTML = `
           <span class="section__item-name">${this.title}</span>
            ...
            <a class="section__item-counter projects__item-counter"
              href="${this.githubLink}">${this.members.length} members
            </a>
          `;
  ...
  function projectsCard() {
    ...
    data.forEach(({id, title, description,members, github_project, programming_language, specialization}) => { 
      new ProjectsCard(id, title, description,members, github_project, programming_language, specialization).render();
      }
    )}).then( () => onResourceLoad() );
